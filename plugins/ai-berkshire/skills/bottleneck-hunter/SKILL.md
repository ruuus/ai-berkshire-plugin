...
**台股标的取数**：供应链扫描常命中台股（4位数字代码，如 2330、3661），其行情/PER/市值/月营收统一用 `python3 {AI_BERKSHIRE_ROOT}/tools/twstock_data.py`（FinMind 数据源，用法见 `../financial-data/SKILL.md` 台股章节）。台股每月10日前强制披露上月营收，月营收同比是验证瓶颈"量价齐升"最快的公开信号。
