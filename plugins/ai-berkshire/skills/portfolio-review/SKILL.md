...
对每个持仓使用 `{AI_BERKSHIRE_ROOT}/tools/financial_rigor.py verify-valuation` 校验估值数据。对每只持仓标注信息丰富度（A/B/C级），C级持仓的分析结论标注低置信度。
...
预期回报估算方法（使用 `{AI_BERKSHIRE_ROOT}/tools/financial_rigor.py three-scenario` 计算）：
